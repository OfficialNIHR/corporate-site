      ...
      <div class="row section-header">
        <div class="col-12 mb1">
          <div>
            <h1>:::title:::</h1>
            <ul class="breadcrumb">
              <a href=":::assetfullpath.shortcutid-756501:::">Researchers</a> > <a href=":::assetfullpath.shortcutid-757185:::">Funding opportunities</a>
            </ul>
          </div>
        </div>
      </div>
        ...
        <xsl:if test="count(Facets) = 0">
              ...
              <div class="SKPosts SKPostsType{Post[1]/PostTypeID}">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <div class="SKPostsNoResults"><h3>We don't currently have any funding opportunities matching these parameters.</h3></div>
                    </xsl:otherwise>
                </xsl:choose>
            </div>
...
<xsl:template match="Post">
  ...
  <xsl:choose><!-- For future funding opportunities -->
    <xsl:when test="Status = 'Future'">
      <div class="row title-list">
              <h2 class="rounded-header rounded-header-style1 rounded-header-lhs rounded-header-future">
                <a href="{RESTURL}">
                  <xsl:choose>
              ...
              <xsl:if test="PostKind = '1' or PostKind = '3'">
                <p class="SKPostsListItemDate">
                  <strong>Potential advert date: </strong><xsl:value-of select="date:format-date(date:dateTime(EndDate), 'dd MMMMM yyyy')"/>
                </p>
              </xsl:if>
    ...
    <xsl:otherwise><!-- For all other funding opportunities -->
        <div class="row title-list">
                ...
                <p class="SKPostsListItemDate">
                    <xsl:choose>
                        <xsl:when test="deadline != ''">
                          <div class="close-icon"></div><strong>Closes: </strong><xsl:value-of select="deadline"/> on <xsl:value-of select="date:format-date(date:dateTime(EndDate), 'dd MMMMM yyyy')"/>
                        </xsl:when>
                        <xsl:otherwise>
                          <div class="close-icon"></div><strong>Closes: </strong><xsl:value-of select="date:format-date(date:dateTime(EndDate), 'dd MMMMM yyyy')"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </p>
              </xsl:if>
